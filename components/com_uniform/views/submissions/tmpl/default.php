...
$paymentStatus = $this->_state->get('filter.filter_payment_status' . $formId);
...
<div id="submissions-list" class="jsn-page-list jsn-master">
	<div class="<?php echo JSNUniformHelper::getBootstrapPrefix(); ?>">
		<form action="<?php echo JRoute::_('index.php?option=com_uniform&view=submissions'); ?>" class="form-inline" method="post" id="adminForm" name="adminForm">
			<fieldset class="jsn-fieldset-filter">
				...
				if ($formId && ($showDataFilter == "1" || $showDateFilter == "1"))
				{
					...
					<div class="pull-left jsn-fieldset-search">
						...
						if ($showDataFilter == "1")
						{
						    ...
						    if (isset($formInfo->form_payment_type) && !empty($formInfo->form_payment_type))
						    {
						        ...
						        try
						        {
						            ...
						            {
						                $paymentCombobox = str_replace('name="filter_payment_status"', 'name="filter_payment_status' . $formId . '"', $renderPaymentStatusComboBox[0]);
						            }
						            $totalDefaultColumns++;
						        }
						        catch(Exception $e)
						...
						<button class="btn btn-icon" type="button" onclick="
						  ...
						  try {
						  		document.id('filter_payment_status').value = '';
                            }
                            catch(err) {
				...
				if ($formId && $showFieldSelector == "1")
				{
					...
					<div id="submission-fields-list" class="jsn-master <?php echo JSNUniformHelper::getBootstrapPrefix(); ?>">
						...
						if ($this->_items)
						{
							...
							<tr class="row<?php echo $i % 2; ?>">
								...
								if (is_array($arrayField))
								{
									...
									foreach ($arrayField as $j => $field)
									{
										...
										if (isset($fieldType[$field]))
										{
											...
											{
											    if (json_decode($contentField_orig, true) == null || version_compare(PHP_VERSION, '5.4.0', '<'))
											    {
											    ...
											    {
											        $contentField_orig =  json_encode(json_decode($contentField_orig), JSON_UNESCAPED_UNICODE);
											    }
											}
								...
								if (isset($formInfo->form_payment_type) && !empty($formInfo->form_payment_type))
								{
								    ...
								    {
								        echo '<td>' . JTEXT::_("JSN_UNIFORM_SELECT_PAYMENT_" . strtoupper($item->log_status)) . '</td>';
								    }

...
<div id="submission-settings" class="<?php echo JSNUniformHelper::getBootstrapPrefix(); ?>">
...
<div id="submission-data" class="submission-data">
	...
	<div class="submission-content">
		<div class="jsn-page-actions btn-group" style="display: block;">
			<button class="btn btn-icon prev-page" onclick="return false;" disabled="disabled">
				<i class="icon-arrow-left"></i></button>
			<button class="btn btn-icon next-page" onclick="return false;" disabled="disabled">
                    ...
                    if (isset($fields->type))
                    {
                        if ($fields->type != 'static-content' && $fields->type != 'google-maps')
                        {
                            ...
                            $submissionEdit .= '<div class="control-group ">
												...
												<div class="controls jsn-uf-' . strtolower($fields->type) . '-field">';
                            $contentField = "";
                            $contentFieldEdit = "";
                            $contentFieldDetail = "";
                            ...
                            else
                            {
                                ...
                                if ($fields->type == "recepient-email")
                                {
                                    if (json_decode($contentFieldDetail, true) == null || version_compare(PHP_VERSION, '5.4.0', '<'))
                                    {
                                        //$contentFieldDetail =  $contentField;
                                    }
                                    else
                                    {
                                        $contentFieldDetail =  json_encode(json_decode($contentFieldDetail), JSON_UNESCAPED_UNICODE);
                                    }
                                }
                            ...
                            else
                            {
                                if ($fields->type == "recepient-email")
                                {
                                    if (json_decode($contentFieldEdit, true) == null || version_compare(PHP_VERSION, '5.4.0', '<'))
                                    {
                                        //do nothing
                                    }
                                    else
                                    {
                                        $contentFieldEdit =  json_encode(json_decode($contentFieldEdit), JSON_UNESCAPED_UNICODE);
                                    }
                                }

...
class JSNUniformModelForm extends JModelItem
{
			...
			foreach ($postFieldIdentifier as $key => $item)
			{
				if (isset($item))
				{
					if ( $fieldType == "likert" ) {
						...
						$data[ $key ] = ( ( version_compare(phpversion(), '7.4.0', '<') && function_exists('get_magic_quotes_gpc') && get_magic_quotes_gpc() == true) || ( version_compare(phpversion(), '7.4.0', '<') && function_exists('get_magic_quotes_runtime') && get_magic_quotes_runtime() == true) ) ? stripslashes( $item ) : $item;
					}
				}
			}
	...
	{
		$postFieldIdentifier = isset($post[$fieldIdentifier]) ? $post[$fieldIdentifier] : '';
		$postFieldIdentifier = ( ( version_compare(phpversion(), '7.4.0', '<') && function_exists('get_magic_quotes_gpc') && get_magic_quotes_gpc() == true) || ( version_compare(phpversion(), '7.4.0', '<') && function_exists('get_magic_quotes_runtime') && get_magic_quotes_runtime() == true) ) ? stripslashes($postFieldIdentifier) : $postFieldIdentifier;
	...
	private function _checkDuplicates($post, $fieldIdentifier, $fieldTitle, &$validationForm)
	{
		...
		$postFieldIdentifier = isset($post[$fieldIdentifier]) ? $post[$fieldIdentifier] : '';
		$postIndentifier = ( ( version_compare(phpversion(), '7.4.0', '<') && function_exists('get_magic_quotes_gpc') && get_magic_quotes_gpc() == true) || ( version_compare(phpversion(), '7.4.0', '<') && function_exists('get_magic_quotes_runtime') && get_magic_quotes_runtime() == true) ) ? stripslashes($postFieldIdentifier) : $postFieldIdentifier;
		if ($postIndentifier)
		{
			...
			if (!in_array($colum->field_id, $fieldClear))
			{
				...
				if ($colum->field_type != 'static-content' && $colum->field_type != 'google-maps')
				{
					if (in_array($colum->field_type, array("single-line-text", "website", "paragraph-text", "country")))
					{
						...
						$postName = ( ( version_compare(phpversion(), '7.4.0', '<') && function_exists('get_magic_quotes_gpc') && get_magic_quotes_gpc() == true) || ( version_compare(phpversion(), '7.4.0', '<') && function_exists('get_magic_quotes_runtime') && get_magic_quotes_runtime() == true) ) ? stripslashes($postFieldName) : $postFieldName;
					...
					if (isset($fieldSettings->options->required) && (int) $fieldSettings->options->required == 1 && (int) $fieldSettings->options->hideField != 1)
					{
						switch($colum->field_type)
						{
							...
							case "email":
								...
								$postEmail = ( ( version_compare(phpversion(), '7.4.0', '<') && function_exists('get_magic_quotes_gpc') && get_magic_quotes_gpc() == true) || ( version_compare(phpversion(), '7.4.0', '<') && function_exists('get_magic_quotes_runtime') && get_magic_quotes_runtime() == true) ) ? stripslashes($postFieldEmail) : $postFieldEmail;
								jimport('joomla.mail.helper');
								$isEmailAddress = JMailHelper::isEmailAddress($postEmail);
								if (!$isEmailAddress)
								{

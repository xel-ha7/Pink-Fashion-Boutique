...
class JSNUniformModelSubmissions extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		$submissionDataForOwner = (bool) $params->get("submission_data_for_owner", 0);
		...
		$paymentStatus 	= $this->getState('filter.filter_payment_status' . $filterFormId);
		$formInfo 		= $this->getInfoForm();

		if (count((array) $formInfo))
		{
		    if ((string) $formInfo->form_payment_type != '')
		    {
		        ...
		        try
		        {
		            ...
		            $rTrigger = $dispatcher->trigger('buildNewPaymentStatusQueryFilter', array(&$query, $paymentStatus));
		            if (!count((array) $rTrigger))
		            {
		...
		if ($submissionDataForOwner)
		{
		    ...
		    {
		        $query->where('us.id = ' . (int) $userID, 'AND');
		    }
		}
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$paymentStatus = $this->getUserStateFromRequest($this->context . '.filter.filter_payment_status' . $filterFormId, 'filter_payment_status' . $filterFormId , '', 'string');
		$this->setState('filter.filter_payment_status' . $filterFormId , $paymentStatus);

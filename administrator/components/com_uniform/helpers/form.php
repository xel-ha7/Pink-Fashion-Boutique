...
class JSNFormGenerateHelper
{
	...
	public static function fieldSlider($data, $submission = null)
	{
		...
		// Generate HTML code.
		$size = 'size="' . strlen($data->options->max) . '"';
		$html = "<div class='control-group {$customClass} {$identify} {$hideField}' data-id='{$data->id}-jsn-uf-form-field'><label class='control-label'>" . htmlentities(JText::_($title), ENT_QUOTES, 'UTF-8') . " {$required} {$instruction}</label><div class='controls'><input class='jsn-uf-slider {$requiredInteger} {$styleClassLimit} {$sizeInput}' id='{$data->id}-jsn-uf-form-field' type='range' name='slider[{$data->id}][value]' value='{$defaultValue}' {$minAttr} {$maxAttr} {$stepAttr} {$limitValue} /> <input type='number' class='jsn-uf-slider-value' value='{$defaultValue}' {$minAttr} {$maxAttr} {$stepAttr} {$size} /></div></div>";

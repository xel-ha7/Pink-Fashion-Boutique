...
class JSNUniFormEmailHelper
{
	...
	public function prepareDataForEmail($post)
	{
		...
		$submissionDetails 		= $this->getSubmissionByID($post['form_id'], $post['submission_id']);
		...
		if (count($templateData))
		{
			foreach ( $templateData as $emailTemplate )
			{
				...
				if ($emailTemplate->template_notify_to == 0 && count ( $formSubmitter ))
				{
					...
					$sent = $this->sendEmail ( $emailTemplate, $listEmailSubmitter, $fileAttach, $submissionDetails );

				}

				if ($emailTemplate->template_notify_to == 1)
				{
					$sent = $this->sendEmail ( $emailTemplate, $emailData, $fileAttach, $submissionDetails );
				}
			}
		}
	}
	 ...
	 */
	public function sendEmail($templateData, $emailList, $attachFile = null, $submissionDetails = array())
	{
		...
		if (!empty($emailList) && is_array($emailList) && count($emailList))
		{
			...
			try
			{
				...
				$body = $emailContent;
				if ($templateData->template_notify_to == 1)
				{
					// Append form location to email message.
					if (count((array) $submissionDetails))
					{
						if (isset($submissionDetails->submission_form_location) && trim($submissionDetails->submission_form_location) != '')
						{
							$body .= sprintf(JText::_('JSN_UNIFORM_FORM_LOCATION'), $submissionDetails->submission_form_location);
						}
					}
				}				
				$sent = "";

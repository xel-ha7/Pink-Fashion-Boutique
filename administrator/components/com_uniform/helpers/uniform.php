...
class JSNUniformHelper
{
	 ...
	 * @return object list
	 */
	public static function getOptionForms($fstate = true)
	{
		$db = JFactory::getDbo();
		$query = $db->getQuery(true);
	...
	public static function setPositionFields($idForm, $data)
	{
		...
		$infoData->field_view = ( ( version_compare(phpversion(), '7.4.0', '<') && function_exists('get_magic_quotes_gpc') && get_magic_quotes_gpc() == true) || ( version_compare(phpversion(), '7.4.0', '<') && function_exists('get_magic_quotes_runtime') && get_magic_quotes_runtime() == true) ) ? stripslashes($data['field_view']) : $data['field_view'];
				...
				if (!empty($submission->$key))
				{
					...
					if ($jsonName)
					{
						...
						if (!empty($jsonName->first) || !empty($jsonName->last) || !empty($jsonName->suffix))
						{
						    ...
						    $contentField = trim(implode(' ', (array) $jsonName));
						}
						else
						{
							$contentField = '';
			...
			case "address":
				if (!empty($submission->$key))
				{
					...
					if ($jsonAddress)
					{
						...
						$contentField = trim(rtrim(implode(', ', (array) $jsonAddress), ','));
					}
					else
					{
						$contentField = $submission->$key;
	...
	public static function generateStylePages($formStyle, $container = "", $containerActive = "", $title = "", $messageErrors = "", $messageBackgroundErrors = "", $field = "")
	{
		if (!empty($container))
		{
			...
			{
				$styleCustom[] = "background:{$formStyle->background_color};";
			}
		...
		if (!empty($containerActive))
		{
			...
			{
				$styleCustom[] = "background:{$formStyle->background_active_color} !important;";
			}
		...
		if (!empty($messageBackgroundErrors))
		{
			...
			{
				$styleCustom[] = "background:{$formStyle->message_error_background_color};";
			}
			$styleCustom[] = "}";
		}
		if ($messageErrors)

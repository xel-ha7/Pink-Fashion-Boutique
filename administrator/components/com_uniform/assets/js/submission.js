        ...
        JSNUniformSubmissionView.prototype = {
            init:function () {
                ...
                $("#jsn-submission-edit").click(function () {
                    ...
                    // Enable disabled edit field.
                    var fields = document.querySelectorAll('.submission-content .submission-page .submission-page-content input[disabled]');

                    for (var i = 0; i < fields.length; i++) {
                        // Mark disabled field.
                        fields[i].setAttribute('data-disabled', '1');

                        // Then, enable it for editing.
                ...
                $("#jsn-submission-save").click(function () {
                    $(".submission-content .submission-page .submission-page-content input").each(function () {
                        ...
                        if (type != "email") {
                            $("dd#sd_" + key).html(self.htmlEntities($(this).val() || 'N/A'));

                            // Show submission details if submission data is not available before but available now.
                            if ($("dd#sd_" + key).hasClass('hide') && $(this).val() != '') {
                                $("dd#sd_" + key).removeClass('hide').prev().removeClass('hide');
                            }

                            // Check if the current field is disabled initially.
                            if (this.getAttribute('data-disabled') && $(this).val() == '') {
                                this.disabled = true;
                            }
                        } else {
                            if ($(this).val()) {

    ...
    function ($, JSNVisualControls, JSNUniformDialogEdition, JSNModal) {
        ...
        JSNVisualDesign.eventChangeSlider = function(event) {
            if (event) {
                ...
                var valueField = document.getElementById('option-value-number');

                if (step && valueField) {
                    step = step.value.indexOf('.') > -1 ? parseFloat(step.value) : parseInt(step.value);

                    if (!isNaN(step) && step > 0) {
                        var value = valueField.value.indexOf('.') > -1
                            ? parseFloat(valueField.value)
                            : parseInt(valueField.value);
                        ...
                        if (remainder != 0) {
                            ...
                            valueField.value = Math.round((Math.round(value / step) * step) * 100) / 100;
                        }
                    }
                }
            }
        ...
        JSNVisualDesign.openOptionsBox = function (sender, type, params, action) {
            ...
            if (type == "form-actions") {
                ...
                if (pageItems.size() <= 1) {
                    ...
                    $("#option-btnNextClass-text").parents(".control-group").remove();
                    $("#option-btnPrev-text").parents(".control-group").remove();
                    $("#option-btnPrevClass-text").parents(".control-group").remove();
                }
            }
            if (type == "currency") {
            ...
            if (type == 'slider') {
                var stepValue = document.getElementById('option-step-number');
                var defaultValue = document.getElementById('option-value-number');

                if (stepValue) {
                    stepValue.onchange = JSNVisualDesign.eventChangeSlider;
                }
            ...
            if ($("#visualdesign-options-general #option-value-textarea").size()) {
                ...
                if ($(defaultEditor).attr('data-editor') == 'tinymce') {
                    ...
                    try 
                    {
                     	var tmpEd = tinyMCE.EditorManager.get('option-value-textarea');
                    	tinyMCE.EditorManager.remove(tmpEd);
                    }
                    catch (err) 
                    {
                        //console.log(err)

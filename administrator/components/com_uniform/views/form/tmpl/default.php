		...
		<div class="span6">
			<fieldset>
				...
				<div class="control-group">
                    ...
                    <div id="recaptcha-msg">
						<?php
						if(!JPluginHelper::isEnabled('captcha','recaptcha')) :

							$captcha_plugin = JSNUniformHelper::checkRecaptchaPlugin();
							$plugin_link = "index.php?option=com_plugins&task=plugin.edit&extension_id={$captcha_plugin->extension_id}";
							?>
                            <div class="msg-1 hidden">
                                <div class="controls recaptcha">
                                    <span class="control-label"><?php echo JText::sprintf('JSN_UNIFORM_FORM_ENABLE_RECAPTCHA', $plugin_link); ?></span>
                                </div>
                            </div>
						<?php
						endif;
						...
						if(!JPluginHelper::isEnabled('captcha','recaptcha_invisible')) :
							...
							$captcha_plugin = JSNUniformHelper::checkRecaptchaPlugin('recaptcha_invisible');
							$plugin_link = "index.php?option=com_plugins&task=plugin.edit&extension_id={$captcha_plugin->extension_id}";
							?>
                            <div class="msg-3 hidden">
                                <div class="controls recaptcha">
                                    <span class="control-label"><?php echo JText::sprintf('JSN_UNIFORM_FORM_ENABLE_RECAPTCHA_INVISIBLE', $plugin_link); ?></span>
                                </div>
                            </div>
						<?php endif; ?>
                    </div>
					<?php if (version_compare(JVERSION, '3.9.0', '<')) : ?>
                    <script type="text/javascript">
                        // Hide the Invisible reCAPTCHA option if Joomla version is older than 3.9.0
                        var selector = document.getElementById('recaptcha-msg').parentNode.querySelector('select');
                        if (selector) {
                            var option = selector.querySelector('option[value="3"]');
                            if (option) {
				...
				<div class="ui-sortable jsn-sortable-disable">
					<div class="form-actions ui-state-default jsn-iconbar-trigger">
						<div class="<?php echo $buttonPosition;?>">
                            ...
                            <input type="hidden" id="jform_form_btn_next_custom_class" name="jsn_form_button[form_btn_next_custom_class]" value="<?php echo !empty($formSettings->form_btn_next_custom_class) ? $formSettings->form_btn_next_custom_class : '';?>">
                            <input type="hidden" id="jform_form_btn_prev_custom_class" name="jsn_form_button[form_btn_prev_custom_class]" value="<?php echo !empty($formSettings->form_btn_prev_custom_class) ? $formSettings->form_btn_prev_custom_class : '';?>">
                            ...
                            <input type="hidden" id="jform_form_btn_reset_custom_class" name="jsn_form_button[form_btn_reset_custom_class]" value="<?php echo !empty($formSettings->form_btn_reset_custom_class) ? $formSettings->form_btn_reset_custom_class : '';?>">
						</div>
						<div class="jsn-iconbar">
		...
		<div class="form-horizontal post-submission-action">
			<fieldset id="postaction">
				...
				<div class="control-group hide" id="form4" style="padding-right: 20px;">
                    ...
                    <div id="custom-msg" class="controls">
                        <div class="js-editor-tinymce">
                            <?php $customMessage = (empty($this->_item->form_id) && in_array($this->formAction, array( '4', '5'))) ? $this->formActionData : $this->actionForm['message']?>
                            <?php
                            if($defaultEditor == 'jce')
                            {
                                $editor = JEditor::getInstance('jce');
                        ...
                        <button class="btn btn-email" id="btn-select-field-message_1" onclick="return false;" title="<?php echo JText::_('JSN_UNIFORM_EMAIL_SETTINGS_INSERT_FIELD'); ?>"><i class="icon-list-2"></i></button>
					</div>
				</div>
				<!--Mailchim Plugin-->
				<?php

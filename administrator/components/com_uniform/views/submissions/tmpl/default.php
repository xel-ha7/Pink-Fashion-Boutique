...
?>
<script>
	var field = [<?php echo ( ( version_compare(phpversion(), '7.4.0', '<') && function_exists('get_magic_quotes_gpc') && get_magic_quotes_gpc() == true) || ( version_compare(phpversion(), '7.4.0', '<') && function_exists('get_magic_quotes_runtime') && get_magic_quotes_runtime() == true) ) ? stripslashes($arrayScriptField) : $arrayScriptField; ?>];
</script>
		...
		<div class="pull-right jsn-fieldset-select">
			...
			if ($formId)
			{
				...
				<select name="filter_form_id" onchange="this.form.submit()" class="inputbox" id="filter_form_id">
					...
					<?php echo JHtml::_('select.options', JSNUniformHelper::getOptionForms(false), 'value', 'text', $formId); ?>
				</select>
...
//							}
					...
					{
                    ?>
                    <th width="100"><?php echo JText::_('JSN_UNIFORM_PAYMENT_STATUS'); ?></th>
                    <?php
					}
					?>
					<th width="30">
...
//										}
							...
							<td>
								<?php
								    ...
								    {
								        echo JTEXT::_("JSN_UNIFORM_SELECT_PAYMENT_" . strtoupper($item->log_status));
								    }
								    else
								    {
								        echo '<a href="index.php?option=com_uniform&view=integration"><i class="icon-question-sign" title="This feature is only avaliable in the latest version of the Payment Gateway. Please update your Payment Gateway"></i></a>';
								    }
                                ?>
							</td>
							<?php  } ?>
					...
					else
					{
						?>
					<tr>
						<td style="text-align: center;" colspan="<?php echo count($arrayField) + $totalDefaultColumns; ?>">
				...
				<tfoot>
				<tr>
					<td colspan="<?php echo count($arrayField) + $totalDefaultColumns; ?>">
						<?php
